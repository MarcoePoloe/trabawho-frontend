// screens/Employer/ApplicantTabScreen.js
...
export default function ApplicantTabScreen() {
  ...
  const [pinned, setPinned] = useState(""); // "pinned", "unpinned", or ""
 ...
 const fetchApplicants = async () => {
  ...
  try {
    ...
    let url = `/employer/applicants?sort_by=${sortBy}&sort_order=${sortOrder}`;
    ...
    // FIX: Extract the applications array from the response
  ...
  const handleApplicantPress = (application) => {
    navigation.navigate("ApplicantDetail", {
      application,
      job: application.jobs,
      applicant: application.job_seekers,
      status: status, 
  ...
  return (
    ...
    >
      <Text style={styles.name}>
        {item.job_seekers?.name || item.applicant?.name || "Unknown Applicant"}
      </Text>
      <Text style={styles.jobTitle}>
        {item.jobs?.title || item.job?.title || "Unknown Job"} — {item.jobs?.company || item.job?.company || ""}
      </Text>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.searchContainer}>
        ...
        <TextInput
          ...
          placeholder="Search applicant name, job title, or company..."
          value={searchInput}
        ...
        <View style={styles.filterRow}>
          ...
          >
            <Ionicons 
              name="filter" 
              size={16} 
              color={(sortBy !== "applied_at" || sortOrder !== "desc") ? "#fff" : "#4A6FA5"} 
            />
            <Text style={[
              styles.filterText,
          ...
          <View style={styles.emptyContainer}>
            ...
            {hasActiveFilters() && (
              <TouchableOpacity onPress={clearAllFilters}>
                <Text style={styles.clearEmptyText}>Clear filters to see all applicants</Text>
              </TouchableOpacity>
            )}
          </View>
        }
...
const styles = StyleSheet.create({
  ...
  jobTitle: { fontSize: 14, color: "#4A6FA5", marginTop: 2 },
